/**
 * pixelation.worker.js
 * 像素化计算 Worker
 * 数据直接内联，不使用 require()（Worker 禁止 require）
 */
...
// 构建指定系统的调色板
function buildPalette(colorSystem) {
  var palette = [];
  var seen = {};
  var entries = Object.entries(COLOR_MAPPING);
  for (var i = 0; i < entries.length; i++) {
    var hex = entries[i][0];
    var systems = entries[i][1];
    var key = systems[colorSystem];
    if (key && !seen[key]) {
      seen[key] = true;
      var rgb = hexToRgb(hex);
      if (rgb) palette.push({ key: key, hex: hex, rgb: rgb });
    }
  }
  return palette;
}
...
// 找最接近的调色板颜色
function findClosest(targetRgb, palette) {
  var minDist = Infinity;
  var closest = palette[0] || { key: '?', hex: '#000000', rgb: { r: 0, g: 0, b: 0 } };
  for (var i = 0; i < palette.length; i++) {
    var c = palette[i];
    var d = colorDistance(targetRgb, c.rgb);
    if (d < minDist) {
      minDist = d;
      closest = c;
    }
...
function getColorKeyByHex(hexValue, colorSystem) {
  var normalized = hexValue.toUpperCase();
  var mapping = COLOR_MAPPING[normalized];
...
function pixelate(params) {
  ...
  var N = params.N;
  var M = params.M;
  var colorSystem = params.colorSystem;
  var mode = params.mode;
  var data = imageData.data;

  var palette = buildPalette(colorSystem);
  ...
  for (var j = 0; j < M; j++) {
    mappedData[j] = [];
    for (var i = 0; i < N; i++) {
      mappedData[j][i] = { key: '?', color: '#FFFFFF', isExternal: false };
    }
  }
  ...
  for (var j = 0; j < M; j++) {
    for (var i = 0; i < N; i++) {
      var sx = Math.floor(i * cellW);
      var sy = Math.floor(j * cellH);
      var ew = Math.max(1, Math.ceil((i + 1) * cellW) - sx);
      var eh = Math.max(1, Math.ceil((j + 1) * cellH) - sy);
      ...
      var rgb;
      if (mode === 'average') {
        rgb = { r: Math.round(rSum / count), g: Math.round(gSum / count), b: Math.round(bSum / count) };
      } else {
        rgb = dominantRgb || { r: 0, g: 0, b: 0 };
      }

      var closest = findClosest(rgb, palette);
      mappedData[j][i] = { key: closest.key, color: closest.hex, isExternal: false };
    }
  }

  // 统计颜色
  var counts = {};
  for (var rj = 0; rj < mappedData.length; rj++) {
    for (var ci = 0; ci < mappedData[rj].length; ci++) {
      var cell = mappedData[rj][ci];
      if (!cell.isExternal && cell.color) {
        if (!counts[cell.color]) counts[cell.color] = { count: 0, color: cell.color };
  ...
  var totalCount = 0;
  var colorKeys = Object.keys(counts);
  for (var k = 0; k < colorKeys.length; k++) {
    totalCount += counts[colorKeys[k]].count;
  }

  var colorList = [];
  for (var ck = 0; ck < colorKeys.length; ck++) {
    var hex = colorKeys[ck];
    colorList.push({
      hex: hex,
      color: hex,
  ...
  return { mappedData: mappedData, counts: counts, colorList: colorList, totalCount: totalCount, N: N, M: M };
}

// 监听主线程消息
onmessage = function(event) {
  var data = event.data;
  if (data.type === 'pixelate') {
    try {
      var result = pixelate(data);
